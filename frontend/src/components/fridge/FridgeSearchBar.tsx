...
const FridgeSearchBar: React.FC<FridgeSearchBarProps> = ({ categories, onSearch }) => {
    ...
    const searchResults = Object.entries(categories).find(
      ([mainCategory, data]) => {
        ...
        if (subCategory) {
          ...
          // 소분류에서 검색
          const detailCategory = Object.entries(data.중분류).find(([_, subCategoryData]) =>
              subCategoryData.소분류 && Object.keys(subCategoryData.소분류).includes(inputValue)
          );
